...
use crate::common::{
    GossipDiscoveryError, IrohGossipDiscoveryResult, Node, NodeInfo, SignedMessage,
};
...
impl GossipDiscoveryReceiver {
    pub fn new(
        ...
        peer_tx: UnboundedSender<NodeId>,
    ...
    pub fn execute_update_map(&self, value: Node) -> IrohGossipDiscoveryResult<()> {
...
mod tests {
    ...
    async fn create_test_receiver(
        ...
        (
            GossipDiscoveryReceiver::new(neighbor_map, peer_tx, receiver, Duration::from_secs(30)),
            peer_rx,
        )
    }
    ...
    async fn test_execute_update_map_empty_name() {
        ...
        receiver.execute_update_map(node.clone()).unwrap();
    ...
    async fn test_execute_update_map_existing_peer_updates_timestamp() {
        ...
        receiver.execute_update_map(node).unwrap();
    ...
    async fn test_execute_update_map_channel_send_error() {
        ...
        let receiver = GossipDiscoveryReceiver::new(
            neighbor_map,
            peer_tx,
            receiver_stream,
        ...
        let iroh_gossip_discovery_result = receiver.execute_update_map(node);
        assert!(iroh_gossip_discovery_result.is_err());
        match iroh_gossip_discovery_result.unwrap_err() {
